				...
				if(typeof(e.data) != 'undefined' && typeof(e.data) == 'string'){
					if(e.data.indexOf('ccmobile_reinitializeauth')!== -1){
						...
						if(controlparameters.type == "extensions" && controlparameters.name == "jabber" && controlparameters.method == "login_gtalk"){
							...
							//notifications in DM
							var options={type:"basic", title: jqcc.cometchat.getLanguage('new_message_from')+controlparameters.params.uname, message: controlparameters.params.message,contextMessage: controlparameters.params.host_name, iconUrl: controlparameters.params.icon};
...
function loadCCPopup(url,name,properties,width,height,title,force,allowmaximize,allowresize,allowpopout,windowmode) {
	...
	if(typeof(windowmode) != "undefined" && windowmode == 1) {
		...
		if(typeof(jqcc.cometchat)!="undefined" && typeof(jqcc.cometchat.getCcvariable) != "undefined" && jqcc.cometchat.getCcvariable().callbackfn=='desktop'){
			...
			if(typeof cc_windownames['cc_'+name] != null){
					...
					if(name != 'passwordBox'){
						if(!$('.cometchat_windows').hasClass('visible') && !$('.cometchat_container').hasClass('visible')){
							if($("#cometchat_righttab").width()-cometchat_container.width() <= 400 && $('#cometchat_righttab').width()!=$(window).width()){
								...
								animatewidth = '0';
								if(embeddedchatroomid >= 1 && $(window).width() > 800){
	...
	if(typeof url != "undefined" && typeof name != "undefined") {
		...
		if(theme == 'embedded'){
			...
			if(chatroommode == 1){
				...
				var messagecontainer = $('#cometchat_righttab').find('#currentroom_convo');
			}else{
				var textfieldheight = jqcc('#cometchat_user_'+toid+'_popup').find('#cometchat_tabinputcontainer').outerHeight(true)+1;
				var messagecontainer = $('#cometchat_righttab').find('#cometchat_tabcontenttext_'+toid);
			}
			var windowtype = '';
			var offset;
...
function outgoingCall(id,grp){
	var theme = jqcc.cometchat.getSettings().theme;
	var baseUrl = jqcc.cometchat.getBaseUrl();
	var buddylistName = jqcc.cometchat.getThemeArray('buddylistName', id);
	var buddylistAvatar = jqcc.cometchat.getThemeArray('buddylistAvatar', id);
	if(typeof(buddylistName) == "undefined"){
		ele = document.getElementById('cometchat_synergy_iframe').contentWindow;
		buddylistAvatar = ele.jqcc.cometchat.getThemeArray('buddylistAvatar',id);
		buddylistName = ele.jqcc.cometchat.getThemeArray('buddylistName', id);
	}
	if(!$("#avchat_container_"+id).length){
		jqcc[theme].playSound(4);
		$('body').append('<div id="avchat_container_'+id+'"><div id="cometchat_avchat_container"><div id="cometchat_userself_left"><div id="cometchat_userself"><span class="cometchat_usersavatar"><img class="cometchat_usersavatarimage" src="'+buddylistAvatar+'" /></span><div id="cometchat_userselfDetails"><div class="avchat_userdisplayname">'+buddylistName+'</div><div class="cometchat_callstatus">Ringing...</div></div></div></div><div class="cometchat_avchat_reject avchat_cancelcall avchat_link_'+grp+'" to='+id+' grp='+grp+'><img src="'+baseUrl+'images/call.svg"></div></div></div>');
	}
}
function incomeingCall(incoming,avchat_data,currenttime){
	var theme = jqcc.cometchat.getSettings().theme;
	var baseUrl = jqcc.cometchat.getBaseUrl();
	var buddylistName = jqcc.cometchat.getThemeArray('buddylistName', incoming.from);
	var buddylistAvatar = jqcc.cometchat.getThemeArray('buddylistAvatar', incoming.from);
	if(typeof(buddylistName) == "undefined"){
		ele = document.getElementById('cometchat_synergy_iframe').contentWindow;
		buddylistAvatar = ele.jqcc.cometchat.getThemeArray('buddylistAvatar',incoming.from);
		buddylistName = ele.jqcc.cometchat.getThemeArray('buddylistName', incoming.from);
	}
	if(!$("#avchat_container_"+incoming.from).length && (incoming.sent > currenttime - 15)){
			$('body').append("<div id='avchat_container_"+incoming.from+"'><div id='cometchat_avchat_container'><div id='cometchat_userself_left'><div id='cometchat_userself'><span class='cometchat_usersavatar'><img class='cometchat_usersavatarimage' src='"+buddylistAvatar+"'/></span><div id='cometchat_userselfDetails'><div class='avchat_userdisplayname'>"+buddylistName+"</div><div class='cometchat_callstatus'>Incoming Call</div></div></div></div><div class='cometchat_avchat_accept acceptAVChat avchat_link_'"+avchat_data[2]+"' to='"+incoming.from+"' token='' grp='"+avchat_data[2]+"' join_url='' start_url='' chatroommode='0' caller='"+avchat_data[3]+"'><img src='"+baseUrl+"images/call.svg'></div><div class='cometchat_avchat_reject avchat_rejectcall avchat_link_"+avchat_data[2]+"' to='"+incoming.from+"' grp='"+avchat_data[2]+"'><img src='"+baseUrl+"images/call.svg'></div></div></div>");
			jqcc[theme].playSound(3);
			var params = {"incoming":incoming.from, "grp":avchat_data[2]};
			setTimeout(function(params) {
				if($("#avchat_container_"+incoming.from).length){
	...
	jqcc('.mediamessage').live('click',function(e){
		if(mobileDevice==null){
			...
			if(((typeof(parent) != 'undefined' && parent != null && parent != self) || window.top != window.self) && jqcc.cometchat.getCcvariable().callbackfn!='desktop'){
				parent.previewCometChatMedia(controlparameters,url);
			} else {
