        ...
        return {
            ...
            initialize: function(){
                ...
                if(settings.showOnlineTab==1 && settings.enableType!=1){
                    if(settings.disableContactsTab == 0){
                        usertab = '<div id = "cometchat_chats" ><span id="cometchat_chatstab" class="cometchat_tab tab_click"><span class="cometchat_tabstext">'+language["contacts"]+'</span></span></div>';
                    }
                    if(hasChatroom){
                        grouptab = '<div id = "cometchat_groups" ><span id="cometchat_groupstab" class="cometchat_tab "><span class="cometchat_tabstext">'+language["groups"]+'</span></span></div>';
                    }
                ...
                if(settings.disableRecentTab == 0 && !chatroomsonly) {
                    recenttab = '<div id = "cometchat_recent" ><span id="cometchat_recenttab" class="cometchat_tab "><span class="cometchat_tabstext">'+language["recent_chats"]+'</span></span></div>';
                ...
                usertabpop = '<div id="cometchat_leftbar">'+tabscontainer+searchbar+'<div id="cometchat_userscontent"><div id="cc_gotoPrevNoti"></div><div id="cc_gotoNextNoti"></div>'+recentpopup+chats+group+'</div></div>'+chatbox;
                var createChatroom = '';
                ...
                $('.cometchat_module').click(function(){
                    ...
                    if(id[2] == 'block'){
                        ...
                        var content = '<iframe src="'+baseUrl+'extensions/bots/index.php?cc_theme=embedded" style="height:100%;border:none;"></iframe>';
            ...
            recentList: function(item){
                ...
                $.each(item, function(i, chat){
                    ...
                    if(chat.grp) {
                        ...
                        var msgcounter = '';
                        if(jqcc.cometchat.getThemeVariable('openChatboxId')!=chat.id && typeof(jqcc.cometchat.getThemeArray('buddylistUnreadMessageCount', chat.id)) != "undefined" && jqcc.cometchat.getThemeArray('buddylistUnreadMessageCount', chat.id) != '') {
            ...
            addMessages: function(item,silent){
                ...
                $.each(item, function(i, incoming){
                    ...
                    if( (!incoming.hasOwnProperty('id') || incoming.id == '') && (!incoming.hasOwnProperty('localmessageid') || incoming.localmessageid == '')){
                        return;
                    }
                    ...
                    if( incoming.hasOwnProperty('localmessageid') ) {
                    ...
                    if(jqcc.cometchat.getThemeArray('buddylistName', incoming.from)==null||jqcc.cometchat.getThemeArray('buddylistName', incoming.from)==''){
                        if(jqcc.cometchat.getThemeArray('trying', incoming.from)<5){
                            setTimeout(function(){
                                if(typeof (jqcc[settings.theme].addMessages)!=='undefined'){
                                    jqcc[settings.theme].addMessages([{"from": incoming.from, "message": message, "self": incoming.self, "old": incoming.old, "id": messagewrapperid, "sent": incoming.sent}]);
                                }
                            }, 2000);
                        }
                    }else{
                        ...
                        if($("#cometchat_message_"+messagewrapperid).length>0){
                            ...
                            if(message.indexOf('avchat_webaction=initiate')!=-1){
                                jqcc.embedded.generateIncomeingAvchatData(incoming,avchat_data,currenttime);
                        ...
                        if(jqcc.cometchat.getThemeVariable('openChatboxId') != incoming.from && incoming.self != 1 && incoming.old != 1 && ((typeof(alreadyreceivedunreadmessages[incoming.from])!='undefined'&& alreadyreceivedunreadmessages[incoming.from]<messagewrapperid) || typeof(alreadyreceivedunreadmessages[incoming.from])=='undefined')){
            ...
            updateReadMessages: function(id){
                if($('#cometchat_user_'+id+'_popup:visible').find('.cometchat_chatboxmessage:not(.cometchat_self):last').length){
                    if(typeof (jqcc.cometchat.updateToStorage)!=='undefined'){
                        ...
                        var messageboxid =  $('#cometchat_user_'+id+'_popup').find('.cometchat_chatboxmessage[id]:not(.cometchat_self):last').attr('id');
            ...
            optionsButton: function(){
                ...
                $(".cometchat_disablelastseen").click(function(event){
                    ...
                    /*if(lastseenflag){
                        ...
                        var status = jqcc.cometchat.getThemeArray('buddylistStatus', jqcc.cometchat.getThemeVariable('openChatboxId'));
                        if(status = 'available' || jqcc.cometchat.getThemeArray('buddylistLastseensetting', jqcc.cometchat.getThemeVariable('openChatboxId')) == 1){
                            jqcc[settings.theme].hideLastseen(jqcc.cometchat.getThemeVariable('openChatboxId'));
                        }
                        else if(status != 'available' && jqcc.cometchat.getThemeArray('buddylistLastseensetting', jqcc.cometchat.getThemeVariable('openChatboxId')) == 0){
...
*/
                ...
                $("#cometchat_lastseen").click(function(event){
                    ...
                    if($("#cometchat_lastseen").is(":checked")){
                        lastseenflag = true;
                        if($("#cometchat_messageElement_"+jqcc.cometchat.getThemeVariable('openChatboxId')).length == 1){
            ...
            openChatTab: function(opentab, restored) {
                ...
                if((opentab == 1 && settings.disableContactsTab == 1) || (opentab == 0 && settings.disableRecentTab == 1) || (opentab == 2 && hasChatroom == 0)){
                    if(settings.disableContactsTab == 0) {
            ...
            windowResize: function(silent){
                if(settings.enableType == 1 && chatroomsonly != 1){
                    ...
                    if(tabcount != 0 && leftbarwidth != null) {
                        var tabswidth = (leftbarwidth/tabcount);
                        tabWidth = 'width: '+tabswidth+ 'px';
                        $("#cometchat_recent").find('.cometchat_tab').attr("style",tabWidth);
            ...
            generateOutgoingAvchatData: function(id,grp){
                if(((typeof(parent) != 'undefined' && parent != null && parent != self) || window.top != window.self) && jqcc.cometchat.getCcvariable().callbackfn!='desktop'){
                    parent.outgoingCall(id,grp);
                } else {
                    outgoingCall(id,grp);
                }
            },
            generateIncomeingAvchatData: function(incoming,avchat_data,currenttime){
                if(((typeof(parent) != 'undefined' && parent != null && parent != self) || window.top != window.self) && jqcc.cometchat.getCcvariable().callbackfn!='desktop'){
                    parent.incomeingCall(incoming,avchat_data,currenttime);
                }else{
                    incomeingCall(incoming,avchat_data,currenttime);
                }
            },
