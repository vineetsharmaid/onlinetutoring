...
function clearcachejscss($directory, $recursive = true, $listDirs = false, $listFiles = true, $exclude = '') {
    global $writable;
    if($handle = opendir(dirname(dirname(__FILE__)).'/writable/'.$writable)) {
        while(false !== ($file = readdir($handle))) {
            if(is_dir(dirname(dirname(__FILE__)).'/writable/'.$writable.'/cache.storage.'.$_SERVER['SERVER_NAME'])){
                rrmdir(dirname(dirname(__FILE__)).'/writable/'.$writable.'/cache.storage.'.$_SERVER['SERVER_NAME']);
            }elseif($file != "." && $file != ".." && $file != "index.html") {
                @unlink(dirname(dirname(__FILE__)).'/writable/'.$writable.'/'.$file);
            }
        }
    }
}
