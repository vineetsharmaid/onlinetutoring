...
if ($action == 'request') {
	...
	if($optionalmessage == 0){
    	$response = sendMessage($to,$whiteboard_language[2]." <a href='javascript:void(0);' class='accept_White' to='".$userid."' random='".$to."' chatroommode='0' mobileAction=\"javascript:jqcc.ccwhiteboard.accept('".$userid."','".$to."');\">".$whiteboard_language[3]."</a> ".$whiteboard_language[4],1);
...
if ($action == 'whiteboard') {
	if(empty($room)){
		$room = md5("whiteboard".$to.rand());
	}
	if (!empty($chatroommode)) {
		if (!empty($subaction)) {
		sendChatroomMessage($to,$whiteboard_language[7]." <a href='javascript:void(0);' class='accept_White' to='".$to."' random='0' room='".$room."' chatroommode='1' mobileAction=\"javascript:jqcc.ccwhiteboard.accept('".$to."','0','".$chatroommode."','".$room."');\">".$whiteboard_language[8]."</a>",0);
		}
	} else {
		if(!empty($random)){
			sendMessage($to,$whiteboard_language[5],2);
			incrementCallback();
			$response = sendMessage($to,$whiteboard_language[2]." <a href='javascript:void(0);' class='accept_White' to='".$userid."' random='".$random."' room='".$room."' chatroommode='0' mobileAction=\"javascript:jqcc.ccwhiteboard.accept('".$userid."','".$random."','".$room."');\">".$whiteboard_language[3]."</a> ".$whiteboard_language[4],1);
			$processedMessage = $_SESSION['cometchat']['user']['n'].": ".$whiteboard_language[2];
			$response['push']=pushMobileNotification($to,$response['id'],$processedMessage);
		}
	}
