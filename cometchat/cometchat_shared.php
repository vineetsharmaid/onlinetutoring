	...
	if (!empty($to) && isset($message) && $message!='' && $userid > 0) {
		...
		if (USE_COMET == 1) {
			...
			if($dir <> 3) {
				...
				$dir_nonself = $comet->publish(array(
					'channel' => $channel,
   		...
   		if(strpos($message,'@') === 0 && $usebots) {
			checkBotMessage($to, $origmessage, 0);
		}
	...
	for ($i=0; $i < $sizeof_broadcast; $i++) {
		...
		if($broadcast[$i]['dir'] != 3 && empty($broadcast[$i]['type'])){
			$info = $comet->publish(array(
				'channel' => $channel,
				'message' => array ( "id" => $insertedid, "from" => $from, "message" => ($broadcast[$i]['message']), "sent" => getTimeStamp(),"self" => $self,"direction" => $broadcast[$i]['dir'],"localmessageid" => $broadcast[$i]['localmessageid'])
				));
		...
		if($broadcast[$i]['dir'] != 1 || $broadcast[$i]['dir'] != 3 && empty($broadcast[$i]['type'])){
			...
			$info = $comet->publish(array(
				'channel' => $channel,
				'message' => array ( "id" => $insertedid, "from" => $from, "message" => ($broadcast[$i]['message']), "sent" => getTimeStamp(),"self" => $self,"direction" => 0,"localmessageid" => $broadcast[$i]['localmessageid'])
				));
		}
	}
}
